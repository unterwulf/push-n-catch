 ...
 */
int sock_init(const char *conffile);
...
int tcp_time_wait(tcp_Socket *s);
 ...
 */
void tcp_cbrk(void (*handler)(void));
...
#define ntohs(netshort)  intel16(netshort)
#define htons(hostshort) intel16(hostshort)
#define ntohl(netlong)   intel(netlong)
#define htonl(hostlong)  intel(hostlong)
 ...
 * see sock_init();
 */
int tcp_config(const char *conffile);
